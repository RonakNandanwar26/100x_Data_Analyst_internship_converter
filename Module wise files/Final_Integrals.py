...
try:
    sheet.add_worksheet(rows=500,cols=200,title='Final Integrals')
except:
    final_integrals = client.open(google_spreadsheet).worksheet('Final Integrals')

final_integrals = client.open(google_spreadsheet).worksheet('Final Integrals')

set_with_dataframe(final_integrals,new_final_integral_df)