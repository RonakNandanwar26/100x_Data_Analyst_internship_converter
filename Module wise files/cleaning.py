...
try:
    sheet.add_worksheet(rows=500,cols=200,title='Cleaned sheet')
except:
    cleaned_sheet = client.open(google_spreadsheet).worksheet('Cleaned sheet') 
...
try:
	sheet.add_worksheet(rows=500,cols=50,title='found Integration')
except:
	sn2an_sheet = client.open(google_spreadsheet).worksheet('found Integration') 

sn2an_sheet = client.open(google_spreadsheet).worksheet('found Integration')

try:
	sheet.add_worksheet(rows=500,cols=50,title='not found Integration')
except:
	sn2an_sheet = client.open(google_spreadsheet).worksheet('not found Integration')

sn2an_not_found_sheet = client.open(google_spreadsheet).worksheet('not found Integration')





set_with_dataframe(cleaned_sheet,final_df)
set_with_dataframe(sn2an_sheet,sn2an_df)
set_with_dataframe(sn2an_not_found_sheet,sn2an_notfound_df)