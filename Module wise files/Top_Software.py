...
try:
    sheet.add_worksheet(rows=1000,cols=50,title='Top SW')
except:
    Top_sw_sheet = client.open(google_spreadsheet).worksheet('Top SW')

Top_sw_sheet = client.open(google_spreadsheet).worksheet('Top SW')
set_with_dataframe(Top_sw_sheet,Top_sw_df)
